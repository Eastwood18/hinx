...
func NewHinxLog(prefix string, flag int) *HinxLoggerCore {
	...
	log := &HinxLoggerCore{prefix: prefix, flag: flag, isolationLevel: 0, calldDepth: 2}
	...
	runtime.SetFinalizer(log, CleanHinxLog)
	return log
}

// CleanHinxLog Recycle log resources
func CleanHinxLog(log *HinxLoggerCore) {
	log.closeFile()
}

func (log *HinxLoggerCore) SetLogHook(f func([]byte)) {
	log.onLogHook = f
}
...
*/
func (log *HinxLoggerCore) formatHeader(t time.Time, file string, line int, level int) {
...
func (log *HinxLoggerCore) OutPut(level int, s string) error {
...
func (log *HinxLoggerCore) verifyLogIsolation(logLevel int) bool {
...
func (log *HinxLoggerCore) Debugf(format string, v ...interface{}) {
...
func (log *HinxLoggerCore) Debug(v ...interface{}) {
	if log.verifyLogIsolation(LogDebug) {
		return
	}
...
func (log *HinxLoggerCore) Infof(format string, v ...interface{}) {
...
func (log *HinxLoggerCore) Info(v ...interface{}) {
	if log.verifyLogIsolation(LogInfo) {
		return
	}
...
func (log *HinxLoggerCore) Warnf(format string, v ...interface{}) {
...
func (log *HinxLoggerCore) Warn(v ...interface{}) {
	if log.verifyLogIsolation(LogWarn) {
		return
	}
...
func (log *HinxLoggerCore) Errorf(format string, v ...interface{}) {
...
func (log *HinxLoggerCore) Error(v ...interface{}) {
	if log.verifyLogIsolation(LogError) {
		return
	}
...
func (log *HinxLoggerCore) Fatalf(format string, v ...interface{}) {
...
func (log *HinxLoggerCore) Fatal(v ...interface{}) {
	if log.verifyLogIsolation(LogFatal) {
		return
	}
...
func (log *HinxLoggerCore) Panicf(format string, v ...interface{}) {
...
func (log *HinxLoggerCore) Panic(v ...interface{}) {
	if log.verifyLogIsolation(LogPanic) {
		return
	}
	s := fmt.Sprintln(v...)
...
func (log *HinxLoggerCore) Stack(v ...interface{}) {
	s := fmt.Sprint(v...)
	s += "\n"
...
// (获取当前日志bitmap标记)
func (log *HinxLoggerCore) Flags() int {
	log.mu.Lock()
	defer log.mu.Unlock()
	return log.flag
}
...
func (log *HinxLoggerCore) ResetFlags(flag int) {
	log.mu.Lock()
	defer log.mu.Unlock()
	log.flag = flag
}
...
// (添加flag标记)
func (log *HinxLoggerCore) AddFlag(flag int) {
	log.mu.Lock()
	defer log.mu.Unlock()
	log.flag |= flag
}
...
// (设置日志的 用户自定义前缀字符串)
func (log *HinxLoggerCore) SetPrefix(prefix string) {
	log.mu.Lock()
	defer log.mu.Unlock()
	log.prefix = prefix
}
...
// (设置日志文件输出)
func (log *HinxLoggerCore) SetLogFile(fileDir string, fileName string) {
	if log.fw != nil {
		log.fw.Close()
	}
...
// SetMaxAge 最大保留天数
func (log *HinxLoggerCore) SetMaxAge(ma int) {
	if log.fw == nil {
		return
	}
	log.mu.Lock()
...
func (log *HinxLoggerCore) SetMaxSize(ms int64) {
	if log.fw == nil {
		return
	}
	log.mu.Lock()
...
// SetCons 同时输出控制台
func (log *HinxLoggerCore) SetCons(b bool) {
	if log.fw == nil {
		return
	}
	log.mu.Lock()
...
// (关闭日志绑定的文件)
func (log *HinxLoggerCore) closeFile() {
	if log.fw != nil {
		log.fw.Close()
	}
}

func (log *HinxLoggerCore) SetLogLevel(logLevel int) {
