...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	hlog.Ins().InfoF("Init timerWhell name = %s is Done!", tw.name)
	return tw
}
...
func (tw *TimeWheel) addTimer(tID uint32, t *Timer, forceNext bool) error {
	defer func() error {
		if err := recover(); err != nil {
			...
			hlog.Ins().ErrorF("addTimer function err : %s", err)
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	hlog.Ins().InfoF("Add timerWhell[%s]'s next [%s] is succ!", tw.name, next.name)
}
...
func (tw *TimeWheel) Run() {
	go tw.run()
	hlog.Ins().InfoF("timerwheel name = %s is running...", tw.name)
}
